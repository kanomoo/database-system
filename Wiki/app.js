...
<div class="callout-card ${conf.class}">
  ...
  function postprocessHtml(html) {
    ...
    result = result.replace(codeBlockRegex, (match, classAttr, codeContent) => {
      ...
      if (lang === 'mermaid') {
        ...
        const nativeVisual = (window.DbNativeVisuals && typeof window.DbNativeVisuals.render === 'function')
          ? window.DbNativeVisuals.render(decodedCode, state.currentDocId || '', diagramIndexInDoc)
          : null;

        if (nativeVisual) {
          return `
            <div class="db-model-card db-native-card">
              <div class="db-model-header">
                <div class="db-model-title-group">
                  <span class="db-term-dots"><i class="dot-red"></i><i class="dot-yellow"></i><i class="dot-green"></i></span>
                  <span class="db-studio-chip">DB LAB MODEL</span>
                  <span class="db-model-title">${escapeHtml(nativeVisual.title)}</span>
                </div>
                <div class="db-model-actions">
                  <a href="../SqlLab/index.html" target="_blank" class="db-model-btn highlight" title="เปิดทดลองใน SQL Lab">
                    ⚡ เปิดใน SQL Lab
                  </a>
                  <button class="db-model-btn btn-diagram-zoom" onclick="window.WikiApp.openDiagramZoom(this)" title="ขยายดูภาพเต็ม">
                    🔍 ขยายภาพ
                  </button>
                </div>
              </div>
              <div class="db-model-viewport">
                ${nativeVisual.html}
              </div>
            </div>
          `;
  ...
  function openDiagramZoom(btn) {
    ...
    if (typeof btn === 'string') {
      contentHtml = btn;
    } else if (btn && btn.tagName === 'IMG') {
      contentHtml = `<img src="${btn.src}" alt="${escapeHtml(btn.alt || '')}" style="max-width:92vw; max-height:85vh; object-fit:contain; border-radius:10px; box-shadow:0 12px 40px rgba(0,0,0,0.5); background:#fff; padding:6px; display:block; margin:auto;">`;
    } else if (btn && btn.closest) {
      const card = btn.closest('.db-model-card, .mermaid-card');
      if (!card) return;
      const viewport = card.querySelector('.db-model-viewport, .mermaid-viewport');
      if (!viewport) return;
      contentHtml = viewport.innerHTML;
    } else {
      return;
    }

    zoomScale = 1;
    elements.diagramCanvas.innerHTML = contentHtml;
    const target = elements.diagramCanvas.firstElementChild;
    if (target) {
  ...
  function setupEventListeners() {
    ...
    // Delegate image and wiki-link clicks inside article-body
